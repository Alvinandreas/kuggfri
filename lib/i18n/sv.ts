...
export const sv = {
  ...
  deck: {
    ...
    modeExam: "Provtenta",
    modeExamHelp: "30 slumpade kort ur urvalet, utan ledtråd och utan att gå tillbaka. Påverkar inte schemat. Visar hur du ligger till.",
    colKnown: "Kan nu",
    colKnownShort: "Kan",
    knownHelp: "Uppskattad andel du kan just nu enligt schemat (aldrig sedda kort räknas som 0).",
    knownTooEarly: "–",
    ...
    sessionPlan: (due: number, fresh: number, minutes: number) => {
      ...
      if (fresh > 0) parts.push(fresh === 1 ? "1 nytt kort" : `${fresh} nya kort`);
      return `${parts.join(" + ")} · cirka ${minutes} min`;
    },
    dailyGoal: "Nya kort per dag",
    dailyGoalHelp: "Förfallna kort kommer alltid med. Det här styr bara hur många nya kort som introduceras per dag.",
    weekdaysOnly: "Jag pluggar helst vardagar",
    weekdaysOnlyHelp: "Helger räknas då inte som missade dagar i din streak.",
    examIn: (days: number) => (days === 0 ? "Tentan är i dag" : days === 1 ? "Tentan är i morgon" : `Tentan om ${days} dagar`),
    examDate: (date: string) => `Tenta ${date}`,
    examPast: "Tentan har varit. Schemat fortsätter långsiktigt, så att kunskapen sitter kvar till nästa kurs.",
  ...
  stats: {
    ...
    streak: "Dagar i rad",
    streakHelp: "Dagar i följd med minst en repetition. Två frysningar täcker enstaka missade dagar; en ny frysning var sjunde aktiva dag.",
  ...
  summary: {
    ...
    examTitle: "Provtentan är klar",
    examScore: (ok: number, total: number, pct: number) => `${ok} av ${total} kort kunde du bra eller direkt (${pct} %).`,
    examNote: "Provtentan rör inte schemat. Kort du skattade 1–3 finns under Kluriga kort och i schemat som vanligt.",
    doneTitle: "Klar för i dag",
    doneBody: "Dagens kort är repeterade. Schemat tar hand om resten: nästa pass i morgon.",
    tileToday: "Kort i dag",
  ...
  admin: {
    ...
    courseCode: "Kurskod",
    examDate: "Tentadatum",
    examDateHelp: "Valfritt. Styr studenternas schema: alla kort hinner repeteras före tentan, de sista två dagarna blir slutrepetition. Lämna tomt om kursen saknar tenta.",
    ...
    activation: "Kommer studenterna tillbaka?",
    activationHelp:
      "Aktivering: andel som gjorde ett riktigt första pass (minst 20 kort) och andel som kom tillbaka inom tre dagar. Räknas bara för studenter vars första dag ligger minst tre dagar tillbaka.",
    activationStarted: "Har börjat",
    activationFirst: "Riktigt första pass",
    activationFirstSub: "minst 20 kort första dagen",
    activationReturned: "Tillbaka inom 3 dagar",
    activationReturnedSub: (eligible: number) => (eligible === 1 ? "av 1 som hunnit" : `av ${eligible} som hunnit`),
