 ...
 *
 * **Allt som körs i molnet är stängt tills någon uttryckligen öppnar det.** Utan
 * `UNDER_UTVECKLING` satt är grinden på i varje Vercel-miljö och av på din egen dator.
 * Det betyder att en deploy aldrig kan råka öppna tjänsten – varken för att en variabel
 * glömts bort, för att produktionsgrenen pekar någon annanstans än man trodde, eller för
 * att ett bygge promotas. Öppning är ett eget beslut: `UNDER_UTVECKLING=0`.
 *
 * Lokalt (ingen `VERCEL`-variabel) är grinden av, så dev-servern, testkopian på 3001 och
 * E2E-sviten påverkas inte. `UNDER_UTVECKLING=1` stänger även lokalt, för den som vill se
 * hur avstängningen ser ut.
 *
 * Varje sida svarar med en enkel sida i stället för appen — gäster, inloggade och admin
 * lika, och även bakgrundsjobben (`/api/cron/*`), så inga mejl går ut medan tjänsten är
 * stängd. Grinden ligger först i middleware och svarar med egen HTML, utan att röra
 ...
 *
 * När tjänsten väl är öppen och stannar öppen: ta bort den här filen och anropet i
 * middleware, hellre än att låta en bortglömd variabel vara det som håller sajten uppe.
 */
...
/**
 * Satt värde avgör. Är inget satt är grinden på i molnet och av lokalt: en deploy ska
 * aldrig kunna öppna tjänsten av misstag, men den lokala utvecklingen ska inte behöva
 * en variabel för att fungera.
 */
export function underUtveckling(): boolean {
  const v = env("UNDER_UTVECKLING").toLowerCase();
  if (v) return !(v === "0" || v === "false" || v === "nej" || v === "av");
  return env("VERCEL") !== "";
}
